<template>
  <form class="resend-activation-link-form" @submit.prevent="onSubmit">
...
import { sendActivationLinkRequest } from '@/requests/user/request'
...
interface ResendActivationLinkForm {
  inputs: Input[]
}
...
const state = reactive<ResendActivationLinkForm>({
...
const onSubmit = () => {
  ...
  const request = sendActivationLinkRequest({
